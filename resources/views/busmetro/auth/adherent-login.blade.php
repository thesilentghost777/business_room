...
<head>
    ...
    <title>Connexion Membre - Business Room</title>
...
<body class="bg-gray-50 min-h-screen flex items-center justify-center px-4">
    <div class="w-full max-w-sm">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Espace Membre</h1>
            <p class="text-sm text-gray-500 mt-1">Connectez-vous à votre compte</p>
        </div>

        @if(session('error'))
        <div class="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm"><i class="fas fa-exclamation-circle mr-1.5"></i>{{ session('error') }}</div>
        @endif

        <form action="{{ route('busmetro.adherent.login.submit') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1.5">Numéro de téléphone</label>
                <input type="tel" name="telephone" value="{{ old('telephone') }}" required autofocus
        ...
        <div class="mt-6 text-center space-y-3">
            <a href="{{ route('busmetro.adherent.register') }}" class="block text-sm text-bm-600 font-medium hover:text-bm-700">
                Pas encore membre ? S'inscrire
            </a>
            <a href="{{ route('busmetro.login') }}" class="text-xs text-gray-400 hover:text-gray-600">
                Espace staff <i class="fas fa-arrow-right ml-1"></i>
            </a>
        </div>
    </div>
</body>
